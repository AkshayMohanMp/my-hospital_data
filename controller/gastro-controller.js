...
const GastroModel = require("../model/gastro-model");




exports.getAllDoctors=async(req, res)=>{
    const Doctor =await GastroModel.find();
...
exports.getSingleDoctorbyId=async (req, res)=>{
    ...
    const Doctors= await GastroModel.findById(id);
    if (!Doctors){
...
exports.AddNewDoctor = async(req, res)=>{
    ...
    const newDoctor = await GastroModel.create({image, name, designation})
...
exports.UpdateDoctor= async(req, res)=>{
    ...
    const{data}= req.body;
    // const Doctors =Doctor.find((each)=> each.id ===id);
    const updateDoctor= await GastroModel.findOneAndUpdate({
...
exports.DeleteDoctorById = async(req,res)=>{
    ...
    const Doctor =await GastroModel.deleteOne({
        _id:id
    })
    // if(!Doctor){
    //     return res.status(404).json({
