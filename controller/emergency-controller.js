...
exports.getAllDoctors=async(req, res)=>{
    const Doctor =await EmergencyModel.find();
...
exports.getSingleDoctorbyId=async (req, res)=>{
    ...
    const Doctors= await EmergencyModel.findById(id);
    if (!Doctors){
...
exports.AddNewDoctor = async(req, res)=>{
    ...
    const newDoctor = await EmergencyModel.create({image, name,designation})
...
exports.UpdateDoctor= async(req, res)=>{
    ...
    const{data}= req.body;
    // const Doctors =Doctor.find((each)=> each.id ===id);
    const updateDoctor= await EmergencyModel.findOneAndUpdate({
...
exports.DeleteDoctorById = async(req,res)=>{
    ...
    const Doctor =await EmergencyModel.deleteOne({
        _id:id
    })
    // if(!Doctor){
    //     return res.status(404).json({
